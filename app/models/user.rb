...
puts 'con ga'
        ...
        if city_result 
          ...
          conditions[:name] = location[0].upcase 
          puts'khoi dau'
          city_result = GeoinfoCity.where(":name LIKE CONCAT(CONVERT('%', CHAR(1)),name,CONVERT('%', CHAR(1)))", conditions).find(:all, :order => "population_2000 desc").first
          city_results = GeoinfoCity.where(":name LIKE CONCAT(CONVERT('%', CHAR(1)),name,CONVERT('%', CHAR(1)))", conditions).find(:all, :order => "population_2000 desc")
          if city_results
